...
public class Direction
{
    ...
    public Direction CombineWith(Direction other)
    {
        ...
        if (YStep == 0)
        {
            direction = new Direction(XStep, other.YStep);
        }
        else
        {
            direction = new Direction(other.XStep, YStep);
        }
    ...
    public ICoordinate GetStartCoordinate(int playMatSize)
    {
        ...
        if (YStep == 1)
        {
            coordinate = XStep == 1 ? new Coordinate(0, 0) : XStep == 0 ? new Coordinate(0, playMatSize/2) : new Coordinate(0, playMatSize - 1);
        }
        else
        {
            coordinate = XStep == 1 ? new Coordinate(playMatSize - 1, 0) : XStep == 0 ? new Coordinate(playMatSize - 1, playMatSize / 2) : new Coordinate(playMatSize - 1, playMatSize - 1);
        }
